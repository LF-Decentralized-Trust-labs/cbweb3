...
type Config struct {
	...
	// same across replicas and across restarts: a per-process value silently rejects
	// every mutating request whose token another instance minted, and the symptom —
	// 403 on some requests but not others — is miserable to diagnose.
	CSRFSecret                  string
