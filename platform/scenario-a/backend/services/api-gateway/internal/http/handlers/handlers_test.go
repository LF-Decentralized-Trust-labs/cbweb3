...
// of REQ-COM-007 (finding R2-H-7, Scenario A parity). When the compliance
// dependency returns an error, the AML gate must NOT fall back to the permissive
// in-memory checker; it must fail closed with 503 Service Unavailable.
func TestAMLScreenFailsClosedOnDependencyError(t *testing.T) {
	t.Parallel()
	// status is APPROVED (non-sanctioned) so a permissive fallback would wrongly
	// return 200; err forces the primary dependency to fail.
...
// (Scenario A parity). PENDING is what the identity service returns for a subject
// it cannot find. A denylist would clear it with a 200; the allowlist blocks it.
