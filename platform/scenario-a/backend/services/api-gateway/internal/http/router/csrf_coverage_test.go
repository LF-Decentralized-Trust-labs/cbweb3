...
func (s allRolesAuthProvider) Validate(_ context.Context, _ string) (domain.TokenClaims, error) {
	return domain.TokenClaims{
		...
		Roles: []string{
			...
			domain.RoleCentralBank, domain.RoleGovernanceOfficer,
		},
	}, nil
}
...
func TestEveryMutatingRouteRequiresCSRF(t *testing.T) {
	...
	//
	// The floor is 12, well below Scenario A's 54 mutating routes, because this
	// harness supplies only the handlers a unit test can cheaply stub — most groups
	// register only when their services are injected. The floor is here to catch an
	// empty route table, not to measure coverage; the structural test below is what
	// proves the routes this harness never registers are covered too.
	if checked < 12 {
		t.Fatalf("swept only %d mutating routes; expected at least 12 — the route table "+
