...
// authenticated token, a treasury operator's included, was served the dashboard. A test
// that mounts the middleware itself passes either way and would not have caught it;
// removing the guard from the route group is the regression that matters, and only this
// level sees it.
//
...
// fakeKeycloak resolves a token string to the roles it carries. An unknown token is
// rejected, which lets a test distinguish 401 (not authenticated) from 403 (authenticated,
// wrong role).
...
func TestPortalGroupRequiresANOCRole(t *testing.T) {
	...
	for _, token := range []string{"noc-admin", "noc-operator", "noc-viewer"} {
		if got := status(t, app, "/api/v1/dashboard", token); got != http.StatusOK {
			t.Errorf("%s must reach the portal group, got %d", token, got)
		}
	}

	if got := status(t, app, "/api/v1/dashboard", "treasury"); got != http.StatusForbidden {
	...
	if got := status(t, app, "/api/v1/dashboard", "no-roles"); got != http.StatusForbidden {
		t.Errorf("a token with no roles must be refused, got %d", got)
	}
	if got := status(t, app, "/api/v1/dashboard", ""); got != http.StatusUnauthorized {
