...
/**
 * One explanation of a failed sign-in, shared by every Scenario A portal.
 *
 * All five portals used to render axios's own `error.message`, so an operator saw "Request failed
 * with status code 400" whether they mistyped a password, left a field empty, or hit a gateway that
 ...
 *
 * These portals sign in with a USERNAME and a PASSWORD — the words the login form uses. The gateway
 * still names the same fields `clientId` and `clientSecret` in its own prose, which is exactly why
 * the operator-facing copy lives here and not there.
 *
 * There is no i18n in these apps, so this is not localised. Keeping every string behind one exported
 * object means a future translation pass has a single seam instead of five `catch` blocks. Naming it
 * is the point; do not inline these strings at call sites.
 */
export const LOGIN_ERROR_COPY = {
  invalidCredentials: "Incorrect username or password. Check both and try again.",
  missingCredentials: "Enter both your username and your password.",
  invalidRequest:
...
const CODE_TO_MESSAGE: Readonly<Record<string, string>> = {
  ...
  // MISSING_REFRESH_TOKEN — the string "refreshToken is required" that a BCRP tester reported seeing
  // on a login screen. Presenting it as a wrong password sends the operator to reset one that works.
 ...
 *
 * An error the app raised itself is returned as written: those messages are deliberate and this
 * mapping has nothing better to say about them.
 */
