...
| — supervisor | [frontend/apps/supervisor/README.md](frontend/apps/supervisor/README.md) | Regulatory oversight portal — Supervisor Portal — In progress |
...
### PKI Trust Model — Central Bank as Spoke CA

> **Warning**: the two paths below are **functionally distinct**. The local path is a development shortcut; the production path is the correct and secure model.

#### Local path — development bootstrap (shortcut)

The `make pki.gen-bank-*` and `make pki.gen-commercial-bank-*` targets generate a **per-commercial-bank** self-signed CA and use it to sign the bank's own certificate. This allows the stack to start locally without a running Central Bank.

```
[DEV ONLY] bank-a-ca.key / bank-a-ca.crt  ← bank's self-signed CA (MUST NOT exist in prod)
           bank-a.key + bank-a.csr         ← bank's keypair
           bank-a.crt                      ← certificate signed by the bank's own CA
```

**This model does not represent production.** Commercial banks must never hold a private CA. The `[DEV ONLY]` warning appears in the terminal every time these targets run.

#### Production path — CB signs the bank's CSR (correct model)

The production model, already implemented in `onboarding_proxy.go` (smart mode), follows this flow:

```
1. Commercial bank generates keypair + CSR (OU=ROLE_COMMERCIAL_BANK).
   → shared/identity/ca.go: GenerateCSR(bankCode, institution, "ROLE_COMMERCIAL_BANK", "BR")

2. Bank submits the CSR to the Central Bank via:
   → POST /api/v1/onboarding/credential-request
   → onboarding_proxy.go: InitiateCredentialRequest() (smart mode — automatically injects csr_pem)

3. Central Bank signs the CSR with the spoke CA (central-bank-x-ca.key):
   → compliance/pki/ca.go: CA.SignCSR(csrPEM)
   → shared/identity/ca.go: SignCSR(caCertPEM, caKeyPEM, csrPEM, years)

4. Bank receives a leaf certificate signed by the CB CA:
   → bank-x.crt  (issuer = central-bank-x-ca.crt, isCA=false)
   → Stored at: PKI_DIR/{bankCode}-participant.crt

5. PoP (Proof-of-Possession) → POST /api/v1/onboarding/complete
   → On-chain registration in IdentityRegistry with the bank's EVM address.
```

**Trust root**: the spoke CA is `central-bank-x-ca.crt` (self-signed by the Central Bank when founding the spoke). It is the only valid issuer of commercial bank certificates. No commercial bank holds or needs its own CA.

#### Correct artifacts per entity

| Entity | Must have | Must not have |
|--------|----------|---------------|
| Central Bank | `central-bank-x-ca.key` + `central-bank-x-ca.crt` (spoke root CA) | — |
| Commercial Bank | `bank-x.key`, `bank-x.csr`, `bank-x.crt` (leaf, issued by CB CA) | `bank-x-ca.key`, `bank-x-ca.crt` |

#### Implementation references

- Smart proxy (commercial bank → CB): [`backend/services/api-gateway/internal/http/handlers/onboarding_proxy.go`](backend/services/api-gateway/internal/http/handlers/onboarding_proxy.go)
- Shared PKI library: [`backend/shared/identity/ca.go`](backend/shared/identity/ca.go) — `GenerateCSR`, `SignCSR`, `GenerateSelfSignedCA`
- Compliance service PKI wrapper: [`backend/services/compliance/internal/pki/ca.go`](backend/services/compliance/internal/pki/ca.go)
- PKI library tests: [`backend/shared/identity/ca_signature_test.go`](backend/shared/identity/ca_signature_test.go)

#### PKI Artifact Audit (production)

To verify that no commercial bank holds its own CA after a toolkit join:

```bash
# Must return empty — no commercial bank should have a CA key in production:
find backend/config/pki/ -name "*-ca.key" | grep -v "central-bank"

# Verify a bank's certificate chain:
openssl verify -CAfile backend/config/pki/central-bank-a-ca.crt backend/config/pki/bank-a.crt
# Expected: backend/config/pki/bank-a.crt: OK
```

> **Note**: In the local dev environment, `bank-a-ca.key` MAY exist (generated by the Makefile shortcut). In production artifacts generated by the provisioning toolkit (TK-9), this file MUST NEVER be present.
