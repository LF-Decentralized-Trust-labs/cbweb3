...
3. [Part A — Commercial Bank: Submitting the Onboarding Request](#3-part-a--commercial-bank-submitting-the-onboarding-request)
   ...
   - [Step 5: Wait for Onboarding Approval](#step-5-wait-for-onboarding-approval)
4. [Part B — Central Bank: Reviewing and Approving the Request](#4-part-b--central-bank-reviewing-and-approving-the-request)
   ...
   - [Step 8: Approve the Pending Onboarding Request](#step-8-approve-the-pending-onboarding-request)
...
| Central Bank Governance Operator | Governance Portal | Reviews, validates, and approves the Onboarding request |
...
```
1. Operator  →  2. Institution  →  3. Onboarding  →  4. Complete
```
...
### Step 5: Wait for Onboarding Approval

After submission, the request enters the **PENDING** state. The wizard transitions to the Onboarding Approval waiting screen.

> ![Step 3 — Onboarding Pending](./screenshots/onboarding/07-step3-onboarding-pending.png)
...
Enter the governance operator's **username** (e.g., `bank-a-client`) and **password**, then click **Sign in**. On success, you land on the Governance Dashboard.
...
2. **Pending Onboarding Approvals** — a live-updated table of commercial banks awaiting Central Bank validation.

> ![Registry Page — Overview](./screenshots/onboarding/10-registry-overview.png)

The **Pending Onboarding Approvals** table refreshes automatically every **15 seconds**. You can also click **Refresh** manually.

---

### Step 8: Approve the Pending Onboarding Request

In the **Pending Onboarding Approvals** table, locate the commercial bank that submitted the onboarding request. Each row shows:
...
> ![Pending Onboarding Approvals Table](./screenshots/onboarding/11-pending-onboarding-table.png)
...
2. Click **Approve Onboarding**.

> ![Onboarding Approved — Success Toast](./screenshots/onboarding/13-onboarding-approved-toast.png)
...
Once the Central Bank has approved the Onboarding request, the bank's portal automatically detects the status change (via polling) and advances to **Step 4**.
...
| `PENDING` | The onboarding request was submitted and is awaiting Central Bank review. | Bank Portal Step 3; Governance Portal Registry (Pending Onboarding table). |
...
| `ONBOARDING_APPROVED` | The Central Bank has approved the Onboarding; final activation is in progress. | Bank Portal Step 4 (finalizing). |
...
### Pending Onboarding table is empty in the Governance Portal

- Click **Refresh** to force a manual poll.
- Confirm the commercial bank completed Step 4 and the wizard shows "Step 3: Onboarding Approval" with a valid Request ID.
