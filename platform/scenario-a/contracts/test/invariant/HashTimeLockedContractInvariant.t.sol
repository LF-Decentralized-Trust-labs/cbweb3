...
/// is what the relay reads to decide whether to release private value.
///
/// Ported from the Scenario B suite deliberately unchanged. The two HTLCs differ in
/// exactly one respect — Scenario A checks the FX-agreement gate BEFORE the timelock,
/// so an expired agreement surfaces its own error regardless of the requested timelock
/// — and that ordering is invisible here, because these invariants construct the HTLC
/// with no FXAgreement and no CommitmentHashRegistry. The rules being pinned (hash
/// gate, timelock, sender-only refund, terminal states, single lock per id) are
/// identical in both trees, so a divergent copy would only invite the two to drift. A lock that
