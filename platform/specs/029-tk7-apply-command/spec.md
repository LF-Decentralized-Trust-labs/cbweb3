...
- O que acontece quando `relay.endpoint` está ausente no manifesto para `mode: found`? → O comando injeta `orchestrator.NoOpRelayRegistrar` no `Deps`; o passo `register-relay` (passo 10) é um passo normal que executa com sucesso como no-op (o registro no relay é simplesmente pulado, não é falha) e o relatório mostra `status: completed`. Quando um `relay.endpoint` ESTÁ configurado, o comando injeta `orchestrator.NewHTTPRelayRegistrar(endpoint)` e o passo `register-relay` FALHA a execução se o relay rejeitar ou estiver inacessível — comportamento herdado do engine.
