...
- [ ] T002 [P] Confirm the governance portal builds and tests clean before changes with `cd scenario-b/frontend && pnpm --filter governance test && pnpm --filter governance build`
...
- [ ] T006 [P] [US1] Write a failing service test asserting `Pause` returns a non-empty tx hash and persists it to `OnChainTxRef`, in `scenario-b/backend/services/api-gateway/internal/services/circuit_breaker_service_test.go`
- [ ] T007 [P] [US1] Write a failing service test asserting `ProposeResume` returns **both** the proposal ID and a distinct tx hash, and persists the hash, in `scenario-b/backend/services/api-gateway/internal/services/circuit_breaker_service_test.go`
- [ ] T008 [P] [US1] Write a failing service test asserting `SignResume` returns a tx hash and persists it, in `scenario-b/backend/services/api-gateway/internal/services/circuit_breaker_service_test.go`
- [ ] T009 [P] [US1] Write a failing service test asserting `GetStatus` reports the pair's most recent action hash ordered by `SignedAt`, including when the latest action was taken by a different institution, in `scenario-b/backend/services/api-gateway/internal/services/circuit_breaker_service_test.go`
- [ ] T010 [P] [US1] Write a failing service test asserting that in no-chain mode all actions still succeed and no hash is produced, in `scenario-b/backend/services/api-gateway/internal/services/circuit_breaker_service_test.go`
...
- [ ] T013 [P] [US1] Write a failing frontend test asserting the Circuit Breaker page renders a returned hash and offers a copy control, and renders a neutral "no on-chain reference" state when absent, under `scenario-b/frontend/apps/governance/src/pages/`
...
- [ ] T024 [P] [US1] Add optional `tx_hash?: string` to the V2 breaker response types, in `scenario-b/frontend/apps/governance/src/types/circuit-breaker-v2.types.ts`
- [ ] T025 [US1] Capture `tx_hash` from all four V2 responses, in `scenario-b/frontend/apps/governance/src/services/api/circuit-breaker-v2.api.ts`
- [ ] T026 [US1] Hold the latest known hash for the selected pair and refresh it from status polling so it survives reload, in `scenario-b/frontend/apps/governance/src/features/circuit-breaker/circuit-breaker-v2.store.ts`
- [ ] T027 [US1] Render the hash as a plain selectable value with a one-click copy control, never as a hyperlink, and never gate the breaker action on it, in `scenario-b/frontend/apps/governance/src/pages/CircuitBreakerPage.tsx`
...
**Checkpoint**: US1 is independently shippable. All tests from T006–T013 pass. Satisfies FR-001 to FR-010.

---

## Phase 4: User Story 2 — One consistent breaker state everywhere (Priority: P2)

**Goal**: The chrome badge and the breaker page can never disagree about whether swaps are halted.

**Independent test**: With the portal in its default configuration, halt a pair and confirm the badge and the page agree.

- [ ] T029 [P] [US2] Write a failing test asserting the breaker badge derives from V2 on-chain status and reports halted when a pair is halted, regardless of the mock toggle, under `scenario-b/frontend/apps/governance/src/`
- [ ] T030 [US2] Repoint the badge state source from the `useMocks`-gated V1 `governanceApi.getCircuitBreaker` to the V2 on-chain status, in `scenario-b/frontend/apps/governance/src/stores/circuit-breaker.store.ts`
- [ ] T031 [US2] Update the badge consumers to read the repointed source, in the Dashboard, Sidebar and AppLayout components under `scenario-b/frontend/apps/governance/src/`
- [ ] T032 [US2] Check whether the V1 `getCircuitBreaker` api and store are now unreferenced and, if so, remove them; if other consumers remain, record why they stay, in `scenario-b/frontend/apps/governance/src/services/api/governance.api.ts`

**Checkpoint**: US2 is independently shippable. Satisfies FR-011 and FR-012.
...
**Checkpoint**: US3 is independently shippable. Satisfies FR-013 to FR-019.
...
- [ ] T044 [P] Run the governance portal tests, lint and build and confirm green with `cd scenario-b/frontend && pnpm --filter governance test && pnpm --filter governance lint && pnpm --filter governance build`
- [ ] T045 [P] Confirm the licence-header gate still passes with `bash tools/check-license-headers.test.sh && bash tools/check-license-headers.sh` (both must exit 0)
- [ ] T046 Walk the manual verification steps on a chain-wired stack and confirm each expectation, per [quickstart.md](./quickstart.md) section 2
- [ ] T047 Split the work into the three commits described in [plan.md](./plan.md) — Scenario B code, then docs plus Scenario B text and config, then Scenario A text and config — so no PR spans both scenarios' code
...
**Story independence**: US1, US2 and US3 are independently deliverable. US3 depends on no code change. US2 touches only the badge path. Only US1 requires the Foundational phase.

**Ordering constraints within US1**: T014 → T015 → T016 → T017 must run in sequence (each depends on the previous signature change compiling). T018–T021 all edit the same file and are therefore sequential. T022 depends on T021.

---

## Parallel execution examples

**Phase 3 tests** — T006, T007, T008, T009, T010 all add cases to the same test file, so treat them as one sequential batch; T013 (frontend) runs in parallel with them.
...
**Phase 6** — T041, T042, T044 and T045 are parallel; T043 and T046 are sequential gates.
...
**Then US3**, which is pure text and configuration and carries no runtime risk, followed by **US2**, which is small but touches shared portal chrome and therefore benefits from being reviewed on its own.
...
| FR-011 badge uses same authoritative source | T029, T030, T031 |
| FR-012 badge cannot show synthetic while page shows live | T029, T030, T032 |
| FR-013 manuals accurately state data source | T033, T035, T039 |
| FR-014 Scenario A manual: live-backed, no removal promise | T039 |
| FR-015 Scenario B governance manual: toggle exists, state default | T033 |
| FR-016 bank manual and settings screen agree | T034, T035 |
| FR-017 summary table agrees and reattributes toggle | T036 |
| FR-018 no inert variables in templates | T037, T040 |
| FR-019 annotate ticket mis-scoping | T038 |
| FR-020 record quorum > 2 limitation, do not fix | T041 |
| FR-021 record missing audit-trail entries, do not fix | T042 |
| FR-022 scenario isolation; Scenario A separable | T005, T039, T040, T047 |
| FR-023 decision model unchanged | T020 (no conflation), T043 (suite green), enforced by review |
