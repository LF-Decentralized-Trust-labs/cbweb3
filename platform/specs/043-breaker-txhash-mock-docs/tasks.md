...
- [ ] T047 Walk the manual verification steps on a chain-wired stack and confirm each expectation, including the rendering not covered by automated tests, per [quickstart.md](./quickstart.md) section 2 — **OUTSTANDING: requires a running chain-wired Scenario B stack; not performed during implementation. This is the only acceptance step still open, and it is the one that covers the rendering no automated test can reach (hash display, copy control, chrome indicator).**
