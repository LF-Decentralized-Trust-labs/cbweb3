...
- **One correction to the source ticket is itself a requirement** (FR-017). The originating review attributes mock-default behaviour to the wrong scenario, and leaving that unrecorded risks the ticket being re-actioned against Scenario A.
- **A known limitation is deliberately documented rather than fixed** (FR-018). Recording it is in scope; fixing it is not.
