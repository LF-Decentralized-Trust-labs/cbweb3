...
- [X] T027 [P] [US1] Approve-flow test: admission-authorized `ApproveKYC` reaches `KYC_APPROVED` + PoP nonce with no chain call. Assert the NEW post-A2 semantics explicitly: `KYC_APPROVED` no longer implies on-chain `Verified` (see T046).
