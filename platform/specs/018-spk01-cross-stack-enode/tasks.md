...
- [X] T036 Update ADR Evidence table in `scenario-a/provisioning/docs/adr-001-cross-stack-enode-addressing.md`: replace "Pending E2E run" with PASS for each T1–T6, including observed values (blockHeight, validator count, peer count, bundle checksum) from the verified run per SC-007 (partial)
- [X] T037 Fix README duplicate section: remove the second `## Flow Execution` header at line 116 and the incomplete bullet list at lines 113–115 in `scenario-a/provisioning/spikes/spk-01-cross-stack-enode/README.md`; preserve the correct flow block at lines 103–111 per SC-006 (partial)
- [X] T038 Add "Hostname rejected in `--bootnodes` (Besu 25.8.0)" entry to ADR Rejected Alternatives in `scenario-a/provisioning/docs/adr-001-cross-stack-enode-addressing.md`: document that Besu 25.8.0 rejects non-IP values in `--bootnodes` at startup, making hostname-based intra-compose bootnodes impossible; record that `validator-entry.sh` DNS resolution via `getent hosts` is the adopted solution per plan 1.8 (missing)
- [ ] T039 Run `make spk01.verify` on Docker Desktop for Mac (or colima) and record results in ADR Evidence table; document any platform differences observed per SC-001 (partial)
