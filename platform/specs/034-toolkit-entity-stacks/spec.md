...
- **Governance Portal:** frontend `governance` + serviço `compliance`; o api-gateway do CB expõe `/api/v1/onboarding/credential-request` (autoriza o banco). `GOVERNANCE_ROLE` on-chain = banco central. Autentica no realm `central-bank-<x>`.
- **NOC:** monitoramento (noc-backend + noc-db próprio + noc-agent + frontend), standalone em `interop/hub-and-spoke/noc/`. Autentica no realm `cbweb3`.
- **Keycloak — NOC e Governance usam o do banco central:** verificado que NOC (realm `cbweb3`) e o backend/governança do CB (realm `central-bank-<x>`) usam a **mesma instância** `cbweb3-keycloak` (porta 8081). Ou seja, a instância de Keycloak co-localizada com o CB serve CB + Governance + NOC (realms distintos).
...
- **FR-001**: O toolkit DEVE levantar, por entidade, uma instância **dedicada** de **Postgres** e **Redis** (isolamento físico). NÃO DEVE usar as instâncias compartilhadas do `deploy/local/compose.yml`.
- **FR-001b (Keycloak — estratégia parametrizável)**: O Keycloak DEVE ser selecionável por um parâmetro de estratégia (concat.md "parameterized com default local"): `keycloak: shared` (default local — uma instância para o spoke, realms por entidade) | `keycloak: per-entity` (cada **banco comercial** com instância própria). Em **qualquer** estratégia, **NOC e Governance usam a instância de Keycloak do banco central** (realms `cbweb3` e `central-bank-<x>`); apenas bancos comerciais ganham instância própria no modo `per-entity`.
...
- **FR-007**: O Keycloak (compartilhado ou per-entity) DEVE ser provisionado com os realms/clients necessários a partir de um template de realm (sem reusar/alterar o `init.sh` legado): a instância do CB hospeda `central-bank-<x>` (CB + Governance) e `cbweb3` (NOC); no modo `per-entity`, a instância de cada banco hospeda o realm `bank-<x>`.
- **FR-008**: O `found` DEVE levantar o stack **NOC** (noc-backend + noc-db + noc-agent + frontend) do spoke, com o agent configurado para os componentes provisionados. O NOC autentica no realm `cbweb3` da instância de Keycloak do banco central (não cria Keycloak próprio).
...
- **FR-014**: O manifesto DEVE expor a estratégia de Keycloak (ex.: `spec.keycloak: shared | per-entity`), com default `shared` no profile `local`. É um parâmetro value/strategy (concat.md §5.4), sem fork de código no engine.

### Key Entities

- **EntityInfra (novo)**: Postgres + Redis dedicados a uma entidade; nomes/portas derivados de spoke/entidade. Keycloak conforme a estratégia (shared/per-entity) — NOC e Governance sempre na instância do CB.
...
- Keycloak é estratégia: `shared` no local (evita N JVMs Keycloak); `per-entity` disponível como opção (prod-shaped). NOC e Governance sempre na instância do CB, independentemente da estratégia.
