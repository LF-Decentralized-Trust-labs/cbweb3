#!/usr/bin/env bash
# tryout-spoke-a-bank-c.sh — Onboard a commercial bank (bank-c) to the Central Bank
#
# Dual-entity 3-phase PKI + Blockchain onboarding flow.
# The script simulates both sides (Bank-C operator and CB governance) and
# exercises the full round-trip across two API Gateways.
#
# Flow (7 steps):
#
#   Step 1  Bank-C operator login (Bank-C Keycloak)
#   Step 2  Credential Request via Bank-C proxy -> CB public endpoint
#   Step 3  Central Bank governance login (CB Keycloak)
#   Step 4  Approve KYC (CB governance)
#   Step 5  Polling via Bank-C proxy (discovers pop_nonce)
#   Step 6  Complete Onboarding via Bank-C proxy (PoP + wallet bind)
#   Step 7  PKI Login directly on CB (validation)
#
# Prerequisites:
#   - Both stacks running: make dev.up (or dev.up-bank-c + dev.up-central-bank-a)
#   - curl, jq, openssl, xxd, cast (Foundry)
#   - backend/config/.env.infra.bank-c    with KC_CLIENT_SECRET
#   - backend/config/.env.infra.central-bank-a with KC_CLIENT_SECRET
#   - backend/config/pki/bank-c.{csr,key}
#
# Usage:
#   ./tryout-spoke-a-bank-c.sh
#
# Environment variables (optional):
#   BANK_URL   Bank-C API base         (default: http://localhost:48080/api/v1)
#   CB_URL     Central Bank API base   (default: http://localhost:38080/api/v1)
#   BANK_ENV   Bank-C .env file        (default: backend/config/.env.infra.bank-c)
...
BANK_URL="${BANK_URL:-http://localhost:48080/api/v1}"
CB_URL="${CB_URL:-http://localhost:38080/api/v1}"
BANK_ENV="${BANK_ENV:-backend/config/.env.infra.bank-c}"
...
require_env_files() {
  if [ ! -f "$BANK_ENV" ]; then
    echo "ERROR: Bank-C env file '$BANK_ENV' not found." >&2
    exit 1
  fi
  if [ ! -f "$CB_ENV" ]; then
...
require_pki_bank_c() {
  if [ ! -f "$PKI_DIR/bank-c.csr" ] || [ ! -f "$PKI_DIR/bank-c.key" ]; then
    echo "ERROR: PKI files not found at $PKI_DIR/bank-c.{csr,key}." >&2
    echo "       Run 'make pki.gen-bank-c pki.gen-commercial-banks' first." >&2
    exit 1
  fi
}
...
# ---------------------------------------------------------------------------
# Step 1 — Bank-C operator login (actor: Bank-C operator)
# Authenticates against Bank-C's Keycloak to get an operator token used
# for the protected proxy routes on Bank-C's gateway.
# ---------------------------------------------------------------------------
...
bank_operator_login() {
  ...
  login_resp=$(curl -s -X POST "$BANK_URL/auth/login" \
    ...
    -d "{\"clientId\": \"bank-c-client\", \"clientSecret\": \"$kc_secret\"}")
  BANK_OPERATOR_TOKEN=$(echo "$login_resp" | jq -r '.accessToken // empty')
  if [ -z "$BANK_OPERATOR_TOKEN" ]; then
    echo "ERROR: Bank-C operator login failed." >&2
...
# ---------------------------------------------------------------------------
# Step 2 — Phase 1: Credential Request (actor: Bank-C operator)
# Calls Bank-C's protected proxy route which forwards to the Central Bank's
...
submit_credential_request() {
  ...
  csr_pem=$(cat "$PKI_DIR/bank-c.csr")
  tmp=$(mktemp)
...
approve_kyc() {
  ...
  resp=$(curl -s -X POST "$CB_URL/compliance/approve-kyc" \
    ...
    -H "Content-Type: application/json" \
    -d "$(jq -n --arg s "$BANK_USER_ID" '{subject: $s, reason: "KYC approved - tryout-spoke-a-bank-c"}')")
...
# ---------------------------------------------------------------------------
# Step 5 — Phase 2.5: Polling (actor: Bank-C operator)
# Calls Bank-C's protected proxy route to discover the KYC approval and
...
# ---------------------------------------------------------------------------
# Step 6 — Phase 3: Complete Onboarding (actor: Bank-C operator)
# Calls Bank-C's protected proxy route. Signs the PoP nonce with secp256k1
...
pki_login() {
  ...
  nonce_sig=$(echo -n "$nonce" | xxd -r -p \
    | openssl dgst -sha256 -sign "$PKI_DIR/bank-c.key" \
    | xxd -p | tr -d '\n')
...
main() {
  ...
  echo ""
  echo "======================================================"
  echo "  Onboarding: bank-c -> Central-Bank-A (3-phase PKI)"
  echo "  Bank-C Gateway : $BANK_URL"
  echo "  Central Bank GW: $CB_URL"
  echo "======================================================"

  echo ""
  echo "--- Actor: BANK-C OPERATOR ---"
  echo "=== [1/7] Bank-C operator login ==="
  bank_operator_login
  echo "  BANK_OPERATOR_TOKEN: ${BANK_OPERATOR_TOKEN:0:60}..."

  echo ""
  echo "=== [2/7] Phase 1 — Credential Request (via Bank-C proxy) ==="
  ...
  echo ""
  echo "--- Actor: BANK-C OPERATOR ---"
  echo "=== [5/7] Phase 2.5 — Polling (via Bank-C proxy) ==="
  poll_onboarding_status
  echo "  Confirmed: status=KYC_APPROVED, pop_nonce present"

  echo ""
  echo "=== [6/7] Phase 3 — Complete Onboarding (via Bank-C proxy) ==="
