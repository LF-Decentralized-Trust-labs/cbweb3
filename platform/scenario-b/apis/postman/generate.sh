...
# Regenerate (or verify) the Scenario B Postman collection from the served OpenAPI spec.
#
...
OUT="$HERE/cbweb3-scenario-b.postman_collection.json"
NAME="CBWeb3 API Gateway — Scenario B — International Hub"
...
if [[ "$MODE" == "check" ]]; then
  ...
  if diff -u <(signature "$OUT") <(signature "$TMP/collection.json") > "$TMP/drift.diff"; then
    echo "OK: the committed Scenario B collection matches the served spec."
  else
