...
// The shared package was the one workspace nothing linted: its lint script echoed that none was
// configured, so "eslint clean on the touched files" was true of the apps and vacuous for anything
// living here — including login-errors.ts, which five portals depend on. Review of #188 caught that.
//
...
export default [
  ...reactConfig,
  {
    ...
    //                          and how the apps import them (`buttonVariants` is re-exported from
    //                          index.ts). Silencing it needs the variants moved to their own files.
    //   chart.tsx              @typescript-eslint/no-explicit-any at line 127.
    //
