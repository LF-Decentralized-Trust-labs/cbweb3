...
//
// A deliberate per-scenario copy of scenario-a/.../@cbweb3/ui (lib/amount.ts), not
// an import: the constitution forbids reaching across scenarios except through
// an explicitly versioned shared library, and there is none. The rule is the
// same in both and any change to one belongs in the other — see
// docs/scenario-drift.md.
//
...
//
// One Scenario B hazard this also closes: `displayToBase` strips every comma as
// a thousands separator, so a raw "1000,10" reaching it becomes 100010 — a
// hundredfold overstatement. Feeding it only the canonical string this module
// produces keeps it away from that path.
