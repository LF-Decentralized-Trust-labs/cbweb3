...
// Approving an institution's KYC is a governance act that the audit trail keeps forever, and the
...
// rule by being refused. These tests pin the rule now that the button and the guard share it.
...
describe("approvalReasonIssue", () => {
  ...
  it("says how far along a short reason is, rather than only that it is short", () => {
    const issue = approvalReasonIssue("too short");
    expect(issue).toContain("9 so far");
  });
