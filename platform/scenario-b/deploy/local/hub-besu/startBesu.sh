...
# distinct genesis/validator keys, distinct port range.
#
...
for folder in tmpFiles/networkFiles/keys/*; do
    case $counter in
    0)
        echo -e "${YELLOW}Copying hub-validator node files...${NC}"
        cp -r "$folder"/* nodes/hub-validator/data/
        ;;
    esac
    counter=$((counter + 1))
done
...
# ─── Start hub-validator node (bootnode / sole validator) ────────────────────
echo -e "${BLUE}Starting hub-validator node (bootnode)...${NC}"
docker run -d \
    --name "${NODE_HUB}" \
    --user root \
    -v "$(pwd)/nodes/hub-validator/data:/opt/besu/data" \
    -v "$(pwd)/genesis:/opt/besu/genesis" \
    -p ${RPC_PORT_HUB}:8545 \
    -p ${WS_PORT_HUB}:8546 \
    -p ${P2P_PORT_HUB}:30303 \
    -p ${P2P_PORT_HUB}:30303/udp \
...
echo -e "${GREEN}hub-validator node started.${NC}"
echo -e "${YELLOW}Waiting 5 seconds for hub-validator node to be ready...${NC}"
sleep 5

# Fetch ENODE from hub-validator node via host port
echo -e "${YELLOW}Fetching ENODE from hub-validator node...${NC}"
max_retries=30
retry_delay=3
retry_count=0
ENODE=""
...
while [ $retry_count -lt $max_retries ]; do
    ENODE=$(curl -s -X POST \
        ...
        "http://127.0.0.1:${RPC_PORT_HUB}" | jq -r '.result')
...
HUB_IP=$(docker inspect \
    -f "{{(index .NetworkSettings.Networks \"${NETWORK_NAME}\").IPAddress}}" \
    "${NODE_HUB}")
ENODE_INTERNAL=$(echo "$ENODE" | sed -e "s/127.0.0.1/${HUB_IP}/g")
