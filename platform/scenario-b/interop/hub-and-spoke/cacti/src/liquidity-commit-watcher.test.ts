...
describe("LiquidityCommitWatcher block watermark (R2-H-11)", () => {
  let dir: string;
  let file: string;

  beforeEach(async () => {
    dir = await fs.mkdtemp(path.join(os.tmpdir(), "lcr-watcher-resume-"));
  ...
  it("resumes from the persisted block instead of startBlock", async () => {
