...
/// @title DeployCBWeb3Test
/// @notice Unit tests for the DeployCBWeb3 master deployment script.
/// @dev Tests verify that all core platform contracts are deployed correctly with proper configuration.
contract DeployCBWeb3Test is Test {
    /// @notice Test deployment of the entire CBWeb3 platform via script
    DeployCBWeb3 public deployScript;
    ...
    function test_ScriptRun_Success() public {
        ...
        assertTrue(tokenBrl.hasRole(DEFAULT_ADMIN_ROLE, expectedAdmin), "TokenBRL admin role not granted");
        assertTrue(tokenBrl.hasRole(CENTRAL_BANK_ROLE, expectedCentralBank), "TokenBRL central bank role not granted");
        ...
        assertTrue(tokenEur.hasRole(DEFAULT_ADMIN_ROLE, expectedAdmin), "TokenEUR admin role not granted");
        assertTrue(tokenEur.hasRole(CENTRAL_BANK_ROLE, expectedCentralBank), "TokenEUR central bank role not granted");
        ...
        assertTrue(amm.hasRole(DEFAULT_ADMIN_ROLE, expectedAdmin), "AMM admin role not granted");
        assertTrue(amm.hasRole(GOVERNANCE_ROLE, expectedGovernance), "AMM governance role not granted");
        ...
        assertFalse(amm.paused(), "AMM should not be paused initially");
