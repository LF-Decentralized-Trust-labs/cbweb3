...
## 6. Register — defects found by LNET-run phases

Status as of 2026-08-21. `DEF-001` … `DEF-006` are the open Phase 4 threshold findings,
analysed in full in [`D12_results.md`](../../D12_results.md) §A.4 and §B.4 — the rows here
do not restate that analysis.
...
| DEF-007 | A | 3 | Major | `deploy/local/keycloak/init.sh`, `tests/integration` | 2026-08-21 | `TestFullHappyPath` fails at `Phase1_Login` (`invalid credentials`) on a clean local bring-up, so phases 2–8 fail on the dependency. The auth service accepts only the OIDC password grant (deliberate, since 2026-06-30) while the test presents client credentials, and the local Keycloak init creates no users to authenticate as instead. Blocks on-demand re-run of the Phase 3 evidence. | Open — provisioning gap; the auth behaviour itself is *not a defect* | [`scenario-a/tests/e2e/README.md`](../../scenario-a/tests/e2e/README.md) |
