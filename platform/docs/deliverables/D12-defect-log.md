...
| DEF-021 | both | 0 | Minor | READMEs, runbooks, plans, catalogs, perf docs | 2026-08-21 | The retirement of the legacy `deploy/local` path (`3b14ecaa`) removed the bring-up and teardown targets, and the documents that instruct them were not updated. The count in the original row (five sites in the D12 plan) was the part that had been looked at, not the extent: the sweep found **79 sites across ~30 documents**, in both scenarios — `make spoke-all`, `spoke-all-down`, `spoke-a`/`spoke-b-down`, `dev.up-*`, `deploy.up-*`/`down-*`, and scenario B's own `scenario-b.up`, `up-infra`, `up-backend`, `down-infra`, `up-perf`. Not a find-and-replace: `frontend-spoke-all` (21 sites) and three `scenario-b.up-*` targets are alive and a blind sweep would have broken them. | **Fixed — awaiting retest**: every pointer now names the toolkit path; the two runbooks' phase procedures keep their existing stale-section banners; dated performance records were left as written. | this branch; `scenario-a/tests/e2e/README.md:42` still names the removed Keycloak init (DEF-007's own remedy) |
| DEF-022 | both | 4 | Major | `tests/performance/lib/stack.sh` (both scenarios) | 2026-08-22 | Neither performance harness can stand a stack up any more. Scenario A's `lib/stack.sh` shells out to `make spoke-a` / `make spoke-all`; scenario B's runs `make scenario-b.up` with `PERF_UP_TARGET` defaulting to `scenario-b.up-perf`. All four targets were removed with the legacy path, so the documented "needs no arguments and no manual setup" behaviour of `perf-all` is gone: the benchmarks only run against a stack somebody else provisioned. Found while sweeping the documentation, not by a test — nothing exercises the bring-up branch. | **Fixed — awaiting retest**: endpoints are now derived from the toolkit manifests (`tests/integration/toolkit-env.sh`) in both harnesses, and the port defaults that pointed at the retired topology were removed rather than re-pointed — an unset endpoint now fails with a named error instead of silently probing a dead port. Provisioning stays the operator's step by default (`PERF_ALLOW_PROVISION=1` opts in, and never passes `--clean`), which is the "must opt into" the original remedy called for | `scenario-a/tests/performance/lib/stack.sh:50-60`; `scenario-b/tests/performance/lib/stack.sh:61`; commit `3b14ecaa` |
