...
Every portal below is wired to a live backend through the API Gateway, with one
exception noted in the table: the **Scenario B** Governance portal has a mock
toggle that is **on by default**. The manuals only describe features that exist in
...
| **Governance** | Live backend (Dashboard + Registry exposed) | Mock by default on the parameters and registry screens; Circuit Breaker always live | Scenario A has no working mock path — its toggle is inert. **Scenario B** consumes `VITE_USE_MOCKS`, read as `!== "false"`, so mock data is served unless it is set to exactly `false` |
