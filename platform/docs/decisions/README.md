...
| [ADR-003](ADR-003-privacidade-no-hub.md) | Privacidade no hub do Scenario B | 9.2 | Proposto — mecanismo decidido (2026-07-28), descope pendente de sign-off |
...
| [ADR-010](ADR-010-replay-degradado-em-controlo-de-conformidade.md) | Repetição degradada num controlo de conformidade (`transfer-limits/restore`) | — | **Aceito (2026-09-10)** — Opção B; implementado |
...
## O que está pendente, e o que isso bloqueia

Os ADRs 001 a 004 estão em **Proposto**, e cada um deles tem um cartão de
implementação que não deve começar antes da decisão — começar antes é construir a
opção que pode ser rejeitada. Cada um dos três primeiros abre com um bloco **Pedido de
decisão** que resume o que se pede, quem assina e o que muda em cada resposta, para que
o sign-off não exija ler o ADR inteiro.
...
- **ADR-003 é o mais próximo de fechar.** A escolha técnica já foi feita em
  2026-07-28 (Paladin no hub é incompatível com o design da Liquidity Pool); falta
  autorizar a exposição que ela implica. É sign-off, não engenharia.
- **A evidência dos três foi reverificada em 2026-08-22** contra `develop` @
  `bfa89aa1`, depois da retirada do caminho legado `deploy/local`. Os achados
  continuam válidos; três citações de arquivo/linha não continuavam e foram
  corrigidas. Um ADR que cita um arquivo inexistente convida o revisor a concluir que
