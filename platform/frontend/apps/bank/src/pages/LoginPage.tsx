...
export function LoginPage() {
  ...
  const form = useForm<LoginForm>({
    ...
    defaultValues: { username: "operator@bank-bra.com", password: "Web3Bank2026!" },
  });

  useEffect(() => {
    if (isAuthenticated) {
      toast("Signed in", {
        description: "Welcome back to the Bank Portal.",
      });
        ...
        <section className="hidden rounded-2xl border border-border/70 bg-card/70 p-8 backdrop-blur lg:block">
          ...
          <div className="mt-8 grid gap-4">
            <div className="flex items-start gap-3 rounded-lg border border-border/80 bg-background/80 p-4">
              ...
              <div>
                <p className="text-sm font-medium">Compliance-first</p>
                <p className="text-xs text-muted-foreground">Structured ZK credential attachment and auditable flows.</p>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-border/80 bg-background/80 p-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Session auth managed via backend HTTP-only cookies.</p>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-border/80 bg-background/80 p-4">
              <Wallet className="mt-0.5 h-5 w-5 text-primary" />
              <div>
                <p className="text-sm font-medium">Wallet-bound operations</p>
                <p className="text-xs text-muted-foreground">Bank operator identity tied to institutional EVM wallet context.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <Card className="mx-auto w-full max-w-md border-border/80 shadow-lg">
          ...
          <CardContent>
            ...
            <p className="text-center text-xs text-muted-foreground">
              Protected environment · RBAC enforced · Cross-border settlement ready
            </p>
          </CardContent>
        </Card>
      </div>
