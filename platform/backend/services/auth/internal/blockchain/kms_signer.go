...
func (s *KMSSigner) SignerAddress(ctx context.Context) (string, error) {
	addr, err := s.KMS.GetAddress(ctx, s.UserID)
	if err != nil {
		return "", fmt.Errorf("kms signer: resolving address for %q: %w", s.UserID, err)
	}
	return addr, nil
}
...
func (s *KMSSigner) SignTx(ctx context.Context, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	result, err := s.KMS.Sign(ctx, s.UserID, hex.EncodeToString(txHash[:]))
	if err != nil {
		return nil, fmt.Errorf("kms signer: signing tx for %q: %w", s.UserID, err)
	}
