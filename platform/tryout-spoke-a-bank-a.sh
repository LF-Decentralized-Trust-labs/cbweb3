#!/usr/bin/env bash
# tryout-spoke-a.sh — Onboard a commercial bank (bank-001) to the Central Bank
#
...
#
# Prerequisites:
#   - Both stacks running: make dev.up (or dev.up-bank-a + dev.up-central-bank)
...
#   - backend/config/pki/bank-001.{csr,key}
#
# Usage:
#   ./tryout-spoke-a.sh
#
...
#   CB_ENV     Central Bank .env file  (default: backend/config/.env.infra.central-bank)
...
CB_ENV="${CB_ENV:-backend/config/.env.infra.central-bank}"
...
submit_credential_request() {
  ...
  code=$(curl -sS -X POST "$BANK_URL/onboarding/initiate" \
    ...
    --data "$(jq -n \
      ...
      --arg inst "Bank 001 S.A." \
      --arg bank "001" \
      --arg country "BR" \
      --arg role "ROLE_COMMERCIAL_BANK" \
      --arg email "ops@bank-001.com.br" \
      --arg user "bank-001" \
      '{
        csr_pem: $csr,
...
main() {
  ...
  echo ""
  echo "======================================================"
  echo "  Onboarding: bank-001 -> Central Bank (3-phase PKI)"
